        ...
        if not receiver_blocked_sender:
            ...
            try:
                ...
                emit('new_message_notification', {
                    'id': 'dm-' + str(message_id), 'type': 'dm', 'sender': sender, 'sender_name': sender_name,
                    'preview': preview,
                    'timestamp': utc_now_iso(), 'last_sender': sender
                }, room=f'user_{receiver}')
...
@socketio.on('forward_file')
def handle_forward_file(data):
    """Forward an already-uploaded file/photo/video to another contact
    without re-uploading it — reuses the existing file_path on disk and
    just creates a new message row + realtime notification for it."""
    try:
        sender = str(data.get('sender', ''))
        receiver = str(data.get('receiver', ''))
        message_type = data.get('message_type', '')
        ...
        if not all([sender, receiver, message_type, file_path]):
            emit('error', {'message': 'Invalid forward data'})
            return
        ...
        try:
            c = conn.cursor()
            c.execute("""
                INSERT INTO messages(sender, receiver, message, message_type, file_path, file_name, file_size, status, timestamp)
                VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (sender, receiver, f"Sent a {message_type}", message_type, file_path, file_name, file_size, "sent", now_iso))
            message_id = c.lastrowid
            c.execute("INSERT OR IGNORE INTO contacts(user_phone, contact_phone, contact_name, last_message, last_sender) VALUES(?, ?, ?, ?, ?)",
                      (sender, receiver, "", f"Sent a {message_type}", sender))
            c.execute("UPDATE contacts SET last_message=?, last_sender=?, timestamp=CURRENT_TIMESTAMP WHERE user_phone=? AND contact_phone=?",
                      (f"Sent a {message_type}", sender, sender, receiver))
            c.execute("INSERT OR IGNORE INTO contacts(user_phone, contact_phone, contact_name, last_message, last_sender) VALUES(?, ?, ?, ?, ?)",
                      (receiver, sender, "", f"Sent a {message_type}", sender))
            c.execute("UPDATE contacts SET last_message=?, last_sender=?, timestamp=CURRENT_TIMESTAMP WHERE user_phone=? AND contact_phone=?",
                      (f"Sent a {message_type}", sender, receiver, sender))
            conn.commit()
        finally:
            return_db_connection(conn)
        ...
        if not receiver_blocked_sender:
            try:
                ...
                emit('new_message_notification', {
                    'id': 'dm-' + str(message_id), 'type': 'dm', 'sender': sender, 'sender_name': sender_name,
                    'preview': preview,
                    'timestamp': now_iso, 'last_sender': sender
                }, room=f'user_{receiver}')
                send_push_to_phone(receiver, sender_name, preview, url='/main')
            except Exception as ne:
                print(f"Error emitting forward notification: {ne}")

    except Exception as e:
        print(f"Error in forward_file: {e}")
        emit('error', {'message': 'Failed to forward file'})
