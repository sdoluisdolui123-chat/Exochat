  ...
  <style>
    * {margin: 0; padding: 0; box-sizing: border-box;}
    ...
    .social-comments-section {
      border-top: 1px solid #f0f7f7; padding: 10px 14px 4px;
    }
    .social-comment-item {
      display: flex; gap: 8px; margin-bottom: 10px; align-items: flex-start;
    }
    .social-comment-bubble {
      background: #f4fafa; border-radius: 12px 12px 12px 3px;
      padding: 8px 12px; flex: 1; font-size: 13px;
      border: 1px solid #daeaea;
    }
    .social-comment-author {
      font-weight: 700; font-size: 12px; color: #0E4950; margin-bottom: 2px;
    }
    .social-comment-input-row {
      display: flex; gap: 8px; align-items: center; padding: 8px 0 10px;
    }
    .social-comment-input {
      flex: 1; padding: 8px 14px; border: 1.5px solid #daeaea;
      border-radius: 20px; font-size: 13px; font-family: inherit;
      outline: none; background: #f4fafa;
    ...
    .social-comment-send {
      ...
      background: #0E4950; color: white; display: flex; align-items: center;
      justify-content: center; cursor: pointer; flex-shrink: 0;
      transition: transform 0.15s, box-shadow 0.15s;
    }
    .social-comment-send:active { transform: scale(0.92); }
    .create-post-card {
    ...
    #socialProfileOverlay {
      ...
      z-index: 8500; flex-direction: column; overflow: hidden;
    ...
    .social-profile-banner {
      height: 110px; background: linear-gradient(135deg,#0E4950,#2ec4b6); position: relative;
    }
...
<body>
  ...
  <script>
    ...
    function renderContacts(contacts) {
      ...
      if (contacts.length === 0) {
        ...
        let contactsHTML = `
          <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
            ...
            <div style="display:flex;gap:16px;align-items:center;">
              <!-- Create Group: chat-bubble group icon — immediately recognisable as "group" -->
              ...
              <!-- Add Contact: person with a plus — standard "add user" icon -->
        ...
        fetch('/api/groups?phone=' + encodeURIComponent(phone))
          ...
          .catch(() => { content.innerHTML = contactsHTML; updateHeaderProfileAvatar(); });
      }
    }
    ...
    function renderProfileOverlay(p) {
      const body = document.getElementById('profileOverlayBody');
      ...
      body.innerHTML = `
        <div class="profile-card">
          ...
          <div class="profile-info-block">
            <div class="profile-display-name">${p.display_name || p.phone}</div>
  ...
  <script>
    ...
    const AVATAR_EMOJIS = ['','😊','😎','🎯','🔥','💫','🌿','🎵','⚡','🦋','🏆','💡','🌙','🎨','🚀'];
    let _profileData = { phone:'', display_name:'', bio:'', avatar_color:'#0E4950', avatar_emoji:'', avatar_photo:'' };
    ...
    function renderProfileTab(p) {
      ...
      content.innerHTML = `
        <div class="profile-card">
          ...
          <input type="file" id="avatarPhotoInput" accept="image/*" style="display:none" onchange="uploadAvatarPhoto(this)">
          <div class="profile-info-block">
            <div class="profile-display-name">${p.display_name || phone}</div>
    ...
    function loadSocialTab() {
      ...
      content.innerHTML = `
        <div class="social-header">
          <div style="display:flex;align-items:center;background:#fff;border:1.5px solid #daeaea;border-radius:12px;padding:7px 12px;gap:8px;width:100%;box-shadow:0 1px 6px rgba(14,73,80,0.07);">
    ...
    function handleSocialSearch(query) {
      ...
      // Filter cached posts and people
      ...
      fetch('/api/social/people?phone=' + encodeURIComponent(_socialPhone()))
        ...
        .then(people => {
          ...
          if (matchedPeople.length > 0) {
            html += `<div style="font-weight:600;color:#0E4950;margin-bottom:8px;font-size:13px;">People</div>`;
            matchedPeople.forEach(p => {
              const av = p.avatar_photo
                ? `<img src="/api/social/avatar/${p.avatar_photo}" style="width:38px;height:38px;border-radius:50%;object-fit:cover;">`
                : `<div style="width:38px;height:38px;border-radius:50%;background:${p.avatar_color||'#0E4950'};display:flex;align-items:center;justify-content:center;color:#fff;font-weight:700;font-size:15px;">${(p.display_name||'?')[0].toUpperCase()}</div>`;
              html += `<div onclick="openSocialProfile('${p.phone}')" style="display:flex;align-items:center;gap:10px;background:#fff;border-radius:14px;padding:10px 14px;margin-bottom:8px;cursor:pointer;border:1px solid #daeaea;">
                ${av}
                <div><div style="font-weight:600;color:#0E4950;">${p.display_name||p.phone}</div>
                <div style="font-size:12px;color:#7aabae;">${p.headline||''}</div></div>
              </div>`;
            });
          }
          if (matchedPosts.length > 0) {
            html += `<div style="font-weight:600;color:#0E4950;margin:10px 0 8px;font-size:13px;">Posts</div>`;
            matchedPosts.forEach(p => {
              html += `<div style="background:#fff;border-radius:14px;padding:12px 14px;margin-bottom:10px;border:1px solid #daeaea;">
                <div style="font-weight:600;color:#0E4950;margin-bottom:4px;">${p.display_name||p.author_phone}</div>
                <div style="font-size:14px;color:#333;">${p.content}</div>
              </div>`;
            });
          }
          if (!html) html = `<div class="placeholder-content"><p style="color:#7aabae;">No results for "<b>${query}</b>"</p></div>`;
          if (sc) sc.innerHTML = html;
        })
        .catch(() => {
          // Fallback: just filter posts
          let html = matchedPosts.map(p => `<div style="background:#fff;border-radius:14px;padding:12px 14px;margin-bottom:10px;border:1px solid #daeaea;">
            <div style="font-weight:600;color:#0E4950;margin-bottom:4px;">${p.display_name||p.author_phone}</div>
            <div style="font-size:14px;color:#333;">${p.content}</div>
          </div>`).join('');
          if (!html) html = `<div class="placeholder-content"><p style="color:#7aabae;">No results for "<b>${query}</b>"</p></div>`;
    ...
    function renderFeed(posts) {
      ...
      let html = renderCreatePostBox(myPhone);
    ...
    function renderCreatePostBox(myPhone) {
      return `
        <div class="create-post-card">
          <div class="create-post-row">
            <div id="cpAvatar" style="width:40px;height:40px;border-radius:50%;background:#0E4950;display:flex;align-items:center;justify-content:center;color:white;font-weight:700;font-size:15px;flex-shrink:0;">?</div>
            <textarea id="newPostText" class="create-post-textarea" placeholder="Share something with your network…" rows="2" maxlength="1000"></textarea>
          </div>
          ...
          <div id="postPhotoPreview" style="display:none;margin-top:8px;position:relative;">
            <img id="postPhotoImg" style="width:100%;max-height:180px;object-fit:cover;border-radius:10px;display:block;" alt="">
    ...
    function renderPostCard(p, myPhone) {
      ...
      const name = p.display_name || p.author_phone;
    ...
    function renderComments(postId, comments) {
      ...
      comments.forEach(c => {
        const name = c.display_name || c.author_phone;
        const avatarHtml = _socialAvatar(c, 30);
        html += `<div class="social-comment-item">
          ${avatarHtml}
          <div class="social-comment-bubble">
            <div class="social-comment-author">${escapeHtml(name)}</div>
            <div style="font-size:13px;color:#1a2e2f;">${escapeHtml(c.content)}</div>
          </div>
        </div>`;
      });
      html += `<div class="social-comment-input-row">
        <input type="text" class="social-comment-input" id="commentInput_${postId}" placeholder="Write a comment…" maxlength="500" onkeydown="if(event.key==='Enter')sendComment(${postId})">
        <button class="social-comment-send" onclick="sendComment(${postId})">
        ...
        people.forEach(p => {
          ...
          html += `<div class="people-card">
            ...
            <div class="people-info" onclick="openSocialProfile('${p.phone}')" style="cursor:pointer;">
              ...
              <div class="people-headline">${escapeHtml(p.headline || p.bio || p.phone)}</div>
            </div>
    ...
    function renderRequestsView(reqs) {
      ...
      if (!reqs || reqs.length === 0) {
        ...
        reqs.forEach(r => {
          const name = r.display_name || r.phone;
          ...
          html += `<div class="conn-req-card" id="req_${r.phone.replace(/\+/g,'_')}">
            ...
            <div class="people-info" onclick="openSocialProfile('${r.phone}')" style="cursor:pointer;">
              <div class="people-name">${escapeHtml(name)}</div>
              <div class="people-headline">${escapeHtml(r.headline || r.bio || r.phone)}</div>
            </div>
    ...
    function renderSocialProfile(p, posts, stats, profilePhone) {
      ...
      const connBtnHtml = isMe ? `
        ...
        : `<button
            id="spConnBtn"
            onclick="sendConnectionRequest('${profilePhone}',this)"
            ${status === 'pending' || status === 'connected' || status === 'following' ? 'disabled' : ''}
            style="padding:10px 22px;border-radius:12px;font-size:14px;font-weight:700;font-family:inherit;cursor:pointer;border:none;
      ...
      document.getElementById('socialProfileBody').innerHTML = `
        ...
        <div style="background:white;border-radius:20px;overflow:hidden;margin-bottom:12px;box-shadow:0 3px 14px rgba(14,73,80,0.09);border:1px solid #daeaea;">
          <!-- Gradient banner -->
          <div style="height:120px;background:linear-gradient(135deg,#0E4950 0%,#2ec4b6 100%);"></div>
          ...
          <div style="padding:16px 18px 18px;display:flex;flex-direction:column;gap:14px;">
            ...
            <div>
              <div style="font-size:22px;font-weight:800;color:#1a2e2f;line-height:1.2;">
                ${escapeHtml(p.display_name || p.user_tag || p.phone)}
              </div>
            ...
            <div style="display:flex;border-top:1px solid #f0f7f7;padding-top:14px;gap:0;">
              ...
              <div style="flex:1;text-align:center;border-right:1px solid #f0f7f7;">
                <div style="font-size:20px;font-weight:800;color:#0E4950;">${stats.followers || 0}</div>
                <div style="font-size:11px;color:#7aabae;font-weight:600;margin-top:2px;">Followers</div>
              </div>
              <div style="flex:1;text-align:center;">
                <div style="font-size:20px;font-weight:800;color:#0E4950;">${stats.following || 0}</div>
                <div style="font-size:11px;color:#7aabae;font-weight:600;margin-top:2px;">Following</div>
              </div>
            </div>
          </div>
        </div>
...
def api_get_profile():
    ...
    try:
        c = conn.cursor()
        c.execute("SELECT phone, display_name, bio, avatar_color, avatar_emoji, last_online, avatar_photo FROM users WHERE phone=?", (phone,))
        row = c.fetchone()
    finally:
        return_db_connection(conn)
    if not row:
...
def _social_user_info(phone, conn):
    ...
    c = conn.cursor()
    c.execute("SELECT phone, display_name, avatar_color, avatar_emoji, avatar_photo, bio, headline, location, website FROM users WHERE phone=?", (phone,))
    row = c.fetchone()
    if not row:
        return {"phone": phone, "display_name": "", "avatar_color": "#0E4950", "avatar_emoji": "", "avatar_photo": "", "bio": "", "headline": "", "location": "", "website": ""}
    return {"phone": row[0], "display_name": row[1] or "", "avatar_color": row[2] or "#0E4950", "avatar_emoji": row[3] or "", "avatar_photo": row[4] or "", "bio": row[5] or "", "headline": row[6] or "", "location": row[7] or "", "website": row[8] or ""}
