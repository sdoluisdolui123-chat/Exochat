...
def init_db():
    ...
    try:
        ...
        try:
            c.execute("SAVEPOINT sp_last_sender")
            c.execute("ALTER TABLE contacts ADD COLUMN last_sender TEXT")
            c.execute("RELEASE SAVEPOINT sp_last_sender")
        except Exception:
            c.execute("ROLLBACK TO SAVEPOINT sp_last_sender")
        c.execute("""
...
<body>
  ...
  <script>
    ...
    window.addEventListener('load', function() {
      ...
      // ── App-wide presence: mark online as soon as the dashboard is open,
      // not just when a specific 1:1 chat is opened ──
      const presencePhone = localStorage.getItem('exomnia_user_phone');
      if (presencePhone) {
        const dashboardSocket = io({ transports: ['websocket', 'polling'] });
        dashboardSocket.on('connect', function() {
          dashboardSocket.emit('app_online', { user: String(presencePhone) });
        });
        setInterval(function() {
          if (dashboardSocket.connected) {
            dashboardSocket.emit('heartbeat', { phone: String(presencePhone) });
          }
        }, 30000);
      }
    });
    ...
    if request.method == "POST":
        ...
        try:
            ...
            try:
                ...
                c.execute("INSERT INTO users(phone,last_online) VALUES(%s,%s) ON CONFLICT(phone) DO NOTHING", (phone, now_iso))
        ...
        try:
            ...
            c.execute("""
                SELECT c.contact_phone, c.contact_name,
                       SUBSTRING(COALESCE(c.last_message,''), 1, 50) ||
                       CASE WHEN LENGTH(COALESCE(c.last_message,'')) > 50 THEN '...' ELSE '' END as last_message,
                       c.last_sender,
...
def api_update_profile():
    ...
    try:
        c = conn.cursor()
        c.execute("""
            UPDATE users SET display_name=%s, bio=%s, avatar_color=%s, avatar_emoji=%s
            WHERE phone=%s
...
@socketio.on('app_online')
def on_app_online(data):
    """Mark a user online app-wide (called from the main dashboard, not just inside a chat)."""
    try:
        user = str(data.get('user', ''))
        if not user:
            return
        connected_users[request.sid] = {'phone': user, 'room': None, 'contact': None}
...
def handle_message(data):
    try:
        ...
        try:
            ...
            message_id = c.fetchone()[0]
            c.execute("INSERT INTO users(phone,last_online) VALUES(%s,%s) ON CONFLICT(phone) DO UPDATE SET last_online=EXCLUDED.last_online", (receiver, now_iso))
            c.execute("INSERT INTO contacts(user_phone,contact_phone,contact_name,last_message,last_sender) VALUES(%s,%s,%s,%s,%s) ON CONFLICT(user_phone, contact_phone) DO UPDATE SET last_message=EXCLUDED.last_message, last_sender=EXCLUDED.last_sender, timestamp=EXCLUDED.timestamp",
    ...
    try:
        ...
        try:
            ...
            c.execute("""
                ...
                FROM groups g
                JOIN group_members gm ON g.id = gm.group_id AND gm.user_phone = %s
        ...
        try:
            ...
            if str(row[0]) != user_phone:
                ...
                c.execute("DELETE FROM message_reactions WHERE message_id IN "
                          "(SELECT id FROM group_messages WHERE group_id=%s)", (group_id,))
    ...
    try:
        ...
        try:
            ...
            if existing:
                ...
                c.execute("INSERT INTO group_members (group_id, user_phone, role) VALUES (%s, %s, 'admin') ON CONFLICT DO NOTHING",
                          (group_id, created_by))
                # Add members
                for m in members:
                    m = str(m).strip()
                    if m and m != created_by:
                        ...
                        c.execute("INSERT INTO group_members (group_id, user_phone) VALUES (%s, %s) ON CONFLICT DO NOTHING",
                                  (group_id, m))
                conn.commit()
        finally:
            return_db_connection(conn)
    ...
    try:
        ...
        try:
            ...
            c.execute("""
                ...
                LEFT JOIN contacts con ON con.user_phone=%s AND con.contact_phone=gm.sender
                WHERE gm.group_id=%s
    ...
    try:
        ...
        try:
            ...
            c.execute("""
                ...
                LEFT JOIN contacts con ON con.user_phone=%s AND con.contact_phone=gm.user_phone
                WHERE gm.group_id=%s
            """, (user_phone, group_id))
    ...
    try:
        ...
        if group_id:
            ...
            c.execute("""SELECT id,sender,receiver,group_id,file_name,file_size,
                         ...
                         FROM voice_messages WHERE group_id=%s
                         ORDER BY timestamp ASC LIMIT %s OFFSET %s""", (group_id, limit, offset))
        else:
...
def social_search():
    ...
    try:
        ...
        c.execute("""SELECT phone, display_name, avatar_color, avatar_emoji, avatar_photo, bio, headline
                     FROM users
                     WHERE (LOWER(display_name) LIKE %s OR phone LIKE %s OR LOWER(headline) LIKE %s OR LOWER(bio) LIKE %s)
