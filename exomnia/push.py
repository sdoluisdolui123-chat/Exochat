...
def push_subscribe():
    ...
    print(f"[push] Subscribed {phone} -> {endpoint[:60]}...")
...
@app.route('/api/push/debug/<phone>')
def push_debug(phone):
    """Quick way to check whether a phone has any stored push
    subscription, without needing to actually trigger a message."""
    conn = get_db_connection()
    try:
        c = conn.cursor()
        c.execute("SELECT endpoint, created_at FROM push_subscriptions WHERE phone=?", (phone,))
        rows = c.fetchall()
    finally:
        return_db_connection(conn)
    return jsonify({
        ...
        "subscriptions": [{"endpoint": r[0][:60] + "...", "created_at": r[1]} for r in rows],
    })
...
def send_push_to_phone(phone, title, body, url='/main', tag=None):
    ...
    try:
        ...
        print(f"[push] pywebpush not available, skipping push notification: {e}")
        return
    ...
    try:
        ...
        subs = c.fetchall()
        print(f"[push] send_push_to_phone({phone!r}): {len(subs)} subscription(s) on file")
        if not subs:
            return
        ...
        for endpoint, p256dh, auth in subs:
            ...
            try:
                ...
                print(f"[push] Sent to {phone} ({endpoint[:50]}...) -> HTTP {getattr(resp, 'status_code', '?')}")
            except WebPushException as e:
                status = getattr(e.response, 'status_code', None)
                print(f"[push] WebPushException for {phone} ({endpoint[:50]}...): HTTP {status} — {e}")
                if status in (404, 410):  # gone / expired subscription
                    dead_endpoints.append(endpoint)
            except Exception as e:
                print(f"[push] Unexpected error sending to {phone}: {type(e).__name__}: {e}")
