...
def _safe_alter(conn, sql):
    """Run an ALTER TABLE in its own transaction; silently skip if column already exists."""
    try:
...
def init_db():
    ...
    try:
        ...
        # ── Step 1: core tables (one big transaction) ──────────────────────────
        c.execute("""
    ...
    # ── Step 2: migrations — each in its own transaction ──────────────────────
    ...
    try:
        for col, default in [
            ...
            _safe_alter(conn2, f"ALTER TABLE users ADD COLUMN {col} TEXT DEFAULT {default}")

        _safe_alter(conn2, "ALTER TABLE messages ADD COLUMN deleted_for TEXT DEFAULT NULL")
        _safe_alter(conn2, "ALTER TABLE contacts ADD COLUMN last_sender TEXT")
    finally:
        ...
        try:
            ...
            c.execute("""
                ...
                WHERE ((m.sender=%s AND m.receiver=%s) OR (m.sender=%s AND m.receiver=%s))
                  AND (m.deleted_for IS NULL OR instr(','||m.deleted_for||',', ','||%s||',') = 0)
            ...
            if existing:
                ...
                for m in members:
                    ...
                    if m and m != created_by:
                        c.execute("INSERT INTO users(phone, last_online) VALUES(%s, %s)",
...
if __name__=="__main__":
    ...
    print("Exomnia Super App on http://0.0.0.0:" + str(port))
    print("Main App: http://0.0.0.0:" + str(port) + "/main")
    print("Chat Login: http://0.0.0.0:" + str(port) + "/")
    print("Security Info: http://0.0.0.0:" + str(port) + "/security")
